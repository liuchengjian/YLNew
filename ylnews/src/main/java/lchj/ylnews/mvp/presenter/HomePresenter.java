...
import lchj.ylnews.mvp.model.HomeRepository;
...
public class HomePresenter extends BasePresenter<HomeRepository> {
    ...
    public HomePresenter(AppComponent appComponent,RxPermissions rxPermissions,NewsAdapter mAdapter) {
    ...
    public void getIndexlist(final Message msg){
...
//        String type = (String) msg.objs[0];
        ...
        mModel.getIndexlist()
                ...
                .subscribe(new ErrorHandleSubscriber<ResultInfo<ResultIndex>>(mErrorHandler) {
                    @Override
                    public void onError(@NonNull Throwable e) {
                        super.onError(e);
                        view.showMessage("获取主页数据失败[" + Utils.getThrowsMsg(e) + "]");
                        newsBeanList.clear();
                        mAdapter.notifyDataSetChanged();
                        msg.handleMessageToTarget();
                    }
                    @Override
                    public void onNext(ResultInfo<ResultIndex> ResultData) {
                        ...
                        if( ResultData.getData().getHeads().size()>0){
                            List<NewsBean> newsHeadsLists = ResultData.getData().getHeads();
                            msg.what = 1;
                            msg.arg1 = 1;

                            msg.obj = newsHeadsLists;
                            msg.handleMessageToTarget();
                        }
                        if( ResultData.getData().getPositions().size()>0){
                            List<NewsBean> jhNewsList = ResultData.getData().getPositions();
                            newsBeanList.clear();
